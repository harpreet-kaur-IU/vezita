...
import AddEvent from '../modules/Modals/AddEvent';
import EventDetails from '../modules/Modals/EventDetails';
...
export default function MyCalendar(){
  ...
  //time slots for days
  const [timesSlots,setTimesSlots] = useState([])
  ...
  useEffect(() => {
    setArr([])
    var year = getDay((selectedDay-1),dayOfMonth).$y;
    var month = ((getDay((selectedDay-1),dayOfMonth).$M)+1).toString();
    var finalMonth = month.length>1?month:"0"+month;
    var day = (getDay((selectedDay+1),dayOfMonth).$D).toString();
    var currentDays = (getDay((selectedDay),dayOfMonth).$D).toString();
    var today = currentDays.length>1?currentDays:"0"+currentDays;
    //end date
    var finalDay = day.length>1?day:"0"+day;
    ...
    var raw = JSON.stringify({
        "startDate": year+"-"+finalMonth+"-"+today,
        "endDate": year+"-"+finalMonth+"-"+finalDay
    });
    ...
    fetch(`${process.env.NEXT_PUBLIC_BASE_URL}calendar-event`, requestOptions)
    ...
    .then(result => {
        ...
        for(var i=0;i<allEvents.length;i++){
            //for start time
            var time = new Date(allEvents[i].start.dateTime);
            var hours = time.getHours();
            var mintues = time.getMinutes();
            var startTime = hours+"."+mintues;

            //for end time
            var time1 = new Date(allEvents[i].end.dateTime);
            var hours1 = time1.getHours();
            var mintues1 = time1.getMinutes();
            var endTime = hours1+"."+mintues1;
            tempArray.push([startTime,endTime])
        }
  ...
  useEffect(() =>{    
    ...
    var raw = JSON.stringify({
        "startDate":dayjs(getWeek(selectedWeek)[0]).format("YYYY-MM-DD"),
        "endDate":dayjs(getWeek(selectedWeek)[6]).format("YYYY-MM-DD")
    });
    ...
    .then(result => {
        var allEvents = result.events;
        var tempArray = [];
        //this loop is for creating array of object for every date, duplicates also allowed
        //this will create single different entries of slot with same date
        var weekArraySlot = [];
        for(var i=0;i<=6;i++){
            var obj = {
                "date":dayjs(getWeek(selectedWeek)[i]).format("YYYY-MM-DD"),
                "slot":[]
            }
            weekArraySlot.push(obj)
        }
        
        for(var i=0;i<allEvents.length;i++){
            var date = dayjs(allEvents[i].start.dateTime).format("YYYY-MM-DD")
            //for start time
            var time = new Date(allEvents[i].start.dateTime);
            var hours = time.getHours();
            var mintues = time.getMinutes();
        ...
        for(var i=0;i<=6;i++){
            for(var j=0;j<tempArray.length;j++){
                if(weekArraySlot[i].date == tempArray[j].date){
                    var slots = weekArraySlot[i].slot;
                    slots.push(tempArray[j].slot)
                }
            }
        }

        //this will bundle the slots with same date
        //e.g. if 12 jan has two slots then there will a object with date and with 2d array of both slots
        // var arry = [];
        // var newArray = [];
        // var obj = {
        //     "date":"",
        ...
        //     if(tempArray[i].date.includes("2023-01-12")){
        //         arry.push(tempArray[i].slot)
        //         obj = {
        ...
        //         obj = {
        //             "date":tempArray[i].date,
        //             "slot":tempArray[i].slot
        //         }
        //     }
        //     newArray.push(obj)
        // }

        //this will remove the duplicate dates obj from array
        //new array will be the final slot array
        // for(var i=0;i<newArray.length;i++){
        //     if(newArray[i].date == "2023-01-12"){
        //         newArray.splice(i,1)
        //     }
        // }
        setWeekSlot(weekArraySlot)
    ...
    //for month data API
    useEffect(()=>{
        console.log(dayjs(getMonth(selectedMonth).daysMatrix[1][3].day).format('YYYY-MM-DD'))
        // var myHeaders = new Headers();
        // myHeaders.append("token",JWTToken);
        // myHeaders.append("Content-Type","application/json");
      ...
      <Base>
                ...
                <div className="d-flex d-justify-space-between d-align-center">
                    <div className='d-flex d-align-center'>
                        ...
                        <div className="d-flex d-align-center">
                            ...
                            <div onClick={addEventHandler} className={`ml-3 cursor-pointer bg-primary d-flex d-align-center gap-1 ${styles["add-new-event"]}`}>
                                <img src='plus-white.png'></img>
                                <h6 className='text-white l-22 f-500'>Add new event</h6>
                            </div>
            ...
            <div className={`d-flex d-flex-wrap col-12 mt-2`}>
                        ...
                        <div className='col-12 ml-1 d-flex d-flex-wrap d-align-start'>
                            <div className={`col-12 p-5 ${styles["events"]}`}>
                                ...
                                <ul>
                                    ...
                                    <li onClick={nowAppointmentHandler} className={`p-3 d-flex d-flex-wrap cursor-pointer gap-3 text-white bg-purple mt-3 rounded-8`}>
                                        <h5 className='f-400 l-22'>12:00</h5>
                                        <h5 className='f-400 l-22'>Video Call with Arlene McCoy</h5>
                                    </li>
                                </ul>

                                <ul className='d-flex d-flex-column gap-2'>
                                    <h5 className='f-500 l-26 text-grey-2 mt-10'>Up next</h5>
                                    <li className={`p-3 d-flex d-flex-wrap cursor-pointer gap-3 text-white bg-purple rounded-8`}>
                                        <h5 className='f-400 l-22'>12:00</h5>
                                        <h5 className='f-400 l-22'>Video Call with Arlene McCoy</h5>
                                    </li>
                                    <li className={`p-3 d-flex d-flex-wrap cursor-pointer gap-3 text-white bg-teal-5 rounded-8`}>
                                        <h5 className='f-400 l-22'>12:00</h5>
                                        <h5 className='f-400 l-22'>Video Call with Arlene McCoy</h5>
                                    </li>
                                </ul>
                ...
                {viewTab == "week" && <div className={`col-12 ${styles["week__calendar"]}`}>
                    <div className={`p-relative ${styles["week__header"]}`}>
                        ...
                        {/* Map this module */}
                        <Week week={currentWeek} message={slotMessage} arraySlots = {weekSlot} handler={weekToDayHandler} />
                        {/* Title, Sub-Title, From Date Object, To Date Object */}
                    </div>
                </div>}
                {viewTab == "day" && <div className={`col-12 ${styles["week__calendar"]}`}>
                    <div className="d-flex d-flex-wrap col-12 mr-2 p-relative">
                        ...
                        <div className="col-12">
                            <WeekDay type="1" arraySlots = {arr} day={currentDay}></WeekDay> 
                        </div>
                    </div>
                </div>}
            </div>
        ...
        {newEvent &&
            <Modal modalClass="modal-verify">
                <AddEvent handler={addEventHandler}></AddEvent>
            </Modal>
        }
        {appointment &&
            <Modal modalClass="modal-verify">
                <EventDetails type="video" handler={nowAppointmentHandler}></EventDetails>
            </Modal>
        }
      </Base>
    </Fragment>
